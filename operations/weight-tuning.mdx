...
description: "Configure and monitor the optional Claude-powered weight tuning system that nightly adapts observability scoring weights based on observation outcomes — with full audit trail and rollback."
---
...
<Steps>
  ...
  <Step title="Apply with safeguards">
    ...
    1. Each weight is clamped to a maximum change of ±`max_delta_per_night` (default 5%) from its current value
    2. The full weight set is renormalized to sum exactly 1.0
    3. If no weight moved more than `min_weight_change` (default 0.5%), the change is considered immaterial and discarded silently
...
tuning:
  ...
  max_delta_per_night: 0.05     # hard cap: no weight moves more than ±5% per night
  min_nights_for_tuning: 7      # require ≥ 7 nights of historical data before first run
  min_measurements: 30          # require ≥ 30 measurements before first run
  min_weight_change: 0.005      # skip apply/notify if no weight moves > 0.5%
```
...
| `lookback_days` | `14` | Number of past nights whose outcomes are analysed in each run. Larger values mean more stable evidence but slower adaptation. |
| `max_delta_per_night` | `0.05` | Trust-region cap: no single weight can change by more than this fraction (0.05 = 5%) in one night, regardless of what Claude proposes. |
| `min_nights_for_tuning` | `7` | Tuning is skipped if fewer than this many nights of data exist. Prevents premature adjustments during beta rollout. |
| `min_measurements` | `30` | Tuning is skipped if fewer than this many measurements exist in the lookback window. |
| `min_weight_change` | `0.005` | If the maximum change across all weights after clamping is smaller than this, the run is treated as a no-op — no DB write, no notification. |
...
<Note>
Both `min_measurements` (default 30) and `min_nights_for_tuning` (default 7) must be satisfied before the monitor makes its first weight change. On a fresh installation with no observation history, expect at least one to two weeks before tuning activates.
</Note>
...
<Accordion title="Weight-tuning monitor is not running">
  ...
  5. **Insufficient data:** Tuning requires at least `min_measurements` (default 30) and `min_nights_for_tuning` (default 7) measurements in the lookback window. New installations will not tune until this threshold is met.
</Accordion>
...
<Accordion title="Claude API errors during tuning">
  ...
  |-------|-------|-----------|
  | `429 Too Many Requests` | Rate limit hit on Anthropic API | Reduce frequency by increasing `lookback_days`, or temporarily disable tuning |
