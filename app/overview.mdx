...
| **More** | Science program suggestions (members can propose new targets/programs via `suggest_program_screen.dart`), settings, and about. |
...
A **Help** tab (reachable from the More menu and the app bar) surfaces contact links, the AI chat assistant, live node diagnostics, and config patch requests.
...
<Tabs>
  ...
  <Tab title="Novel">
    ...
    - **Spatial audio sky map** — hold the phone up and hear which objects are overhead, powered by `sensors_plus`
...
| `sensors_plus` | Accelerometer for the spatial audio sky map |
...
| `geolocator ^13.0.0` | Location for node registration |
